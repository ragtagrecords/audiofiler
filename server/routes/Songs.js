...
const { addSongToPlaylist, deleteSongFromPlaylist } = require('../services/SongPlaylists.js');
...
async function updateSongVersions(db, newSongID, parentID, isParent) {
    ...
    if (!isParent) {
        ...
        playlists.forEach(async({id}) => {
            ...
            songRemoved = await deleteSongFromPlaylist(db, parentID, id);
            ...
            newSongAdded = await addSongToPlaylist(db, newSongID, id);
...
exports.addSong = (async function (req ,res) {
    ...
    if (playlistIDs) {
        for (let i = 0; i < playlistIDs.length; i += 1) {
            newSongPlaylistID = await addSongToPlaylist(
                db,
                newSongID,
                playlistIDs[i]
            );
...
exports.updateSong = (async function (req ,res) {
    ...
    // Remove song fields that shouldn't be there
    if  (song.position !== undefined) {
        delete song.position;
    }

    // Attempt to update in DB
    let resultMessage = await SongSvc.updateSong(db, id, song);
